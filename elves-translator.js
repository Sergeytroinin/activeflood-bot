...
module.exports = function fromElvesToRussian(message) {
  let result = message.toLowerCase().replace(escapedChars,'').split(/\s+/);
  let countError = 0;
  for(let i = 0; i < result.length; i++){
    if(errorWords[result[i]])
      countError++;
  }
  if (countError < countErrors)
    return message;
  result='';
  for(let i = 0; i < message.length; i++){
    result += arrReplace[message.charAt(i)] || message.charAt(i);
  }
  for(let i in expectWord){
